                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                    ...
                    <div
                        ...
                        <div class="flex space-x-2 items-center">
                            <button @click="
                                    openViewNotulensi = true;
                                    notulensiTitle = 'Notulensi {{ $rabes->judul }}';
                                    notulensiViewUrl = '{{ $rabes->lampiran ? route('notulensi.view', $rabes->id) : '' }}';
                                    notulensiDownloadUrl = '{{ $rabes->lampiran ? asset('storage/' . $rabes->lampiran) : '' }}';
                                " 
                                class="bg-[#105e75] hover:bg-[#0b4354] text-white px-4 py-1.5 md:py-2 rounded-lg text-xs md:text-sm font-bold transition cursor-pointer">Lihat</button>
                            <form action="{{ route('notulensi.destroy', $rabes->id) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="text-[#105e75] hover:text-red-600 md:hover:bg-red-50 p-1.5 rounded-md transition cursor-pointer">
