...
<tr x-data="{ openBukti: false, openPdf: false, openImg: false }" 
    class="flex flex-col md:table-row border-b md:border-b border-gray-100 hover:bg-gray-50 transition-colors bg-white mb-4 md:mb-0 rounded-lg md:rounded-none shadow-sm md:shadow-none p-4 md:p-0">
    ...
    <!-- Kolom Jam Tap -->
    <td class="py-2 md:py-3 md:px-4 flex items-center justify-between md:table-cell text-gray-900 font-medium text-[15px]">
        <span class="md:hidden text-xs font-bold text-slate-500 uppercase">Jam Tap</span>
        {{ $item['jam_tap'] }}
    </td>
    ...
    <!-- Kolom Aksi -->
    <td class="py-3 md:px-4 flex items-center justify-between md:table-cell mt-2 md:mt-0 border-t md:border-0 border-gray-100">
        <span class="md:hidden text-xs font-bold text-slate-500 uppercase">Aksi</span>
        <button type="button" @click="openBukti = true" class="px-3 py-1.5 bg-[#0c5970] text-white text-xs font-semibold rounded hover:bg-[#084254] transition-colors shadow-sm">
            Lihat Bukti
        </button>
    </td>
    ...
    <template x-teleport="body">
        <div x-show="openBukti" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm" style="display: none;" x-transition>
            <div @click.outside="openBukti = false" class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl relative">
                <button type="button" @click="openBukti = false" class="absolute top-4 right-4 text-gray-400 hover:text-gray-700">
                ...
                <div class="space-y-3 text-sm text-gray-600">
                    ...
                    @if($item['status'] === 'Tidak Hadir')
                        ...
                        <div class="pt-4 grid grid-cols-2 gap-3 border-t mt-2">
                            <!-- PASTIKAN ADA type="button" DISINI -->
                            <button type="button" @click="openPdf = true" class="w-full py-2 bg-blue-50 text-blue-700 font-semibold rounded-lg border border-blue-200 hover:bg-blue-100 transition flex items-center justify-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                Surat Izin
                            </button>
                            <button type="button" @click="openImg = true" class="w-full py-2 bg-green-50 text-green-700 font-semibold rounded-lg border border-green-200 hover:bg-green-100 transition flex items-center justify-center gap-1">
                    ...
                    @else
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900 font-bold text-[#0ea53b]">{{ $item['jam_tap'] }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-medium text-gray-500">Discan oleh</span>
                            <span class="text-gray-900">{{ $item['scanned_by'] ?? '-' }}</span>
                        </div>
                    @endif
                </div>
            </div>
