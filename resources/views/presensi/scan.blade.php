...
<head>
    ...
    <title>Scanner Presensi - {{ config('app.name') }}</title>
...
<body class="h-full bg-black overflow-hidden select-none font-poppins text-slate-100">
    ...
    <div x-data="scannerApp()" x-init="initScanner()" class="relative w-screen h-screen flex justify-center items-center">
        ...
        <div x-show="toast.show"
             ...
             :class="toast.type === 'error' ? 'bg-red-600/90 border-red-500 text-white backdrop-blur-md' : 'bg-emerald-600/90 border-emerald-500 text-white backdrop-blur-md'"
             style="display: none;">
        ...
        <div class="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none p-4">
            <div class="w-64 h-64 sm:w-72 sm:h-72 border-2 border-indigo-500 border-dashed rounded-3xl relative flex flex-col justify-center items-center shadow-[0_0_25px_rgba(99,102,241,0.35)] bg-indigo-950/10 backdrop-blur-[1px]">
                <div class="w-full h-0.5 bg-indigo-400 absolute top-0 animate-pulse"></div>
                <span class="text-white/80 text-xs font-bold tracking-widest uppercase bg-black/40 px-3 py-1.5 rounded-full border border-white/10">
                    Arahkan QR Code
                </span>
            </div>

            @if(isset($kegiatanAktif) && $kegiatanAktif)
                <div class="mt-6 px-4 py-2 bg-slate-900/80 backdrop-blur-md rounded-xl border border-slate-700 text-center max-w-xs">
                    <p class="text-[10px] text-gray-400 uppercase font-semibold">Sesi Kegiatan Aktif</p>
                    <p class="text-xs font-bold text-indigo-400 truncate">{{ $kegiatanAktif->judul }}</p>
                </div>
            @endif
        </div>
        ...
        <button @click="takeSnapshot()" :disabled="isLoading" 
                class="absolute bottom-10 z-20 px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-full shadow-[0_0_20px_rgba(79,70,229,0.5)] active:scale-95 transition flex items-center gap-2 disabled:opacity-60 cursor-pointer">
        ...
        {{-- ============================================================= --}}
        <div x-show="showPopup" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div @click.away="denyScan()" 
                ...
                {{-- Foto User --}}
                <div class="mx-auto w-24 h-24 mb-4 rounded-full p-1 bg-indigo-600 shadow-md">
                    <img :src="scanData.photo" class="w-full h-full rounded-full object-cover border-2 border-slate-900" alt="Foto Profil" />
                </div>
                
                {{-- Detail Identitas --}}
                <p class="text-[11px] font-mono text-indigo-400 font-semibold tracking-wider uppercase mb-0.5" x-text="'NIM: ' + scanData.nim"></p>
                ...
                <div class="flex gap-2.5">
                    ...
                    <button @click="acceptScan()" :disabled="isSaving" 
                            class="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-xl shadow-sm transition disabled:opacity-50 flex justify-center items-center cursor-pointer">
    ...
    <script>
        function scannerApp() {
            return {
                ...
                isScanning: false,
                stream: null,
                kegiatanId: {{ $kegiatanAktif?->id ?? 'null' }},
                scanData: { id: '', nama: '', nim: '', divisi: '', photo: '' },
                ...
                async acceptScan() {
                    ...
                    try {
                        const response = await fetch("{{ route('api.scan.record') }}", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                ...
                closePopup() {
                    ...
                    this.scanData = { id: '', nama: '', nim: '', divisi: '', photo: '' };
                    this.resumeScanning();
                },
