<x-app-layout :$title>
    <div class="py-8 font-poppins min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
        <div class="max-w-md mx-auto px-4">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[32px] p-6 sm:p-8 shadow-sm border border-slate-100 dark:border-slate-700/80">
                ...
                @if (auth()->user()->canScanPresensi() || auth()->user()->canViewPanitiaList())
                    <div class="mt-5 grid grid-cols-2 gap-2">
                        @if (auth()->user()->canScanPresensi())
                            <a href="{{ route('presensi.scan') }}"
                                class="px-3 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-800 dark:text-white text-xs font-bold rounded-xl text-center transition">
                                Scan QR
                            </a>
                        @endif
