    ...
    <div class="py-8 font-poppins min-h-screen bg-gray-50 dark:bg-slate-900 transition-colors duration-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Kelola QR Code Pengguna</h1>
                    <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">Monitoring status token dan reset QR Code untuk presensi panitia & peserta</p>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <form action="{{ route('admin.qr.regenerate-all') }}" method="POST"
                          onsubmit="return confirm('PENTING: Re-generate SEMUA QR akan membuat QR lama tidak valid lagi. Lanjutkan?')">
                        @csrf
                        <button type="submit"
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="text-sm font-semibold text-gray-900 dark:text-slate-100">{{ $user->nama }}</div>
                                                <div class="text-xs text-gray-400 dark:text-slate-500">{{ $user->email ?? 'NIM: ' . $user->nim }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    {{-- NIM --}}
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-700 dark:text-slate-300 font-mono font-medium">{{ $user->nim }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        @else
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 dark:bg-red-950/60 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-xs font-semibold rounded-full">
                                    ...
                                    {{-- Actions --}}
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <form action="{{ route('admin.qr.regenerate', $user) }}" method="POST"
                                              class="inline"
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-gray-400 dark:text-slate-500">
                                        <div class="flex flex-col items-center gap-2">
                                            ...
                                            <span class="text-sm font-semibold">Belum ada data pengguna aktif.</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm">
                    <div class="text-xs text-gray-400 dark:text-slate-400 font-semibold uppercase tracking-wider">Total Pengguna Aktif</div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm">
                    ...
                    <div class="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">
                        {{ \App\Models\User::where('status', true)->whereNotNull('qr_token')->count() }}
                    </div>
                </div>
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm">
                    ...
                    <div class="text-2xl font-bold text-red-500 dark:text-red-400 mt-1">
                        {{ \App\Models\User::where('status', true)->whereNull('qr_token')->count() }}
                    </div>
                </div>
            </div>
